        ...
        <table class="table table-bordered table-hover" width="100%" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th class="text-center bg-light">Lembur</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($rekapAbsensi as $rekap)
                    <tr>
                        ...
                        <td class="text-center">
                            {{-- @php
                                $statusHariIni = $absensiHariIni[$rekap->id]['status_kehadiran'] ?? 'Hadir';
                                $statusLembur = $absensiHariIni[$rekap->id]['status_lembur'] ?? 'tidak';
                            @endphp --}}
                            @php
                                $statusHariIni = $absensiHariIni[$rekap->id]['status_kehadiran'] ?? 'Hadir';
                                $statusLembur = $absensiHariIni[$rekap->id]['status_lembur'] ?? 'Tidak';
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status[{{ $rekap->id }}]"
                                    id="hadir_{{ $rekap->id }}" value="Hadir"
                                    {{ $statusHariIni == 'Hadir' ? 'checked' : '' }}>
                                <label class="form-check-label" for="hadir_{{ $rekap->id }}">H</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status[{{ $rekap->id }}]"
                                    id="sakit_{{ $rekap->id }}" value="Sakit"
                                    {{ $statusHariIni == 'Sakit' ? 'checked' : '' }}>
                                <label class="form-check-label" for="sakit_{{ $rekap->id }}">S</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status[{{ $rekap->id }}]"
                                    id="alpha_{{ $rekap->id }}" value="Alpha"
                                    {{ $statusHariIni == 'Alpha' ? 'checked' : '' }}>
                                <label class="form-check-label" for="alpha_{{ $rekap->id }}">A</label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="custom-control custom-switch">
                                {{-- <input type="checkbox" class="custom-control-input" id="lembur_{{ $rekap->id }}"
                                    name="lembur[{{ $rekap->id }}]" value="ya"
                                    {{ $statusLembur == 'ya' ? 'checked' : '' }}> --}}
                                <input type="checkbox" class="custom-control-input" id="lembur_{{ $rekap->id }}"
                                    name="lembur[{{ $rekap->id }}]" value="Ya"
                                    {{ $statusLembur === 'Ya' ? 'checked' : '' }}>
                                <label class="custom-control-label" for="lembur_{{ $rekap->id }}"></label>
                            </div>
                        </td>
                    </tr>
                @empty
